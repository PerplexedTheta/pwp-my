...
sub build {
    ...
    chdir $config->{srcdir}
        or $logger->fatal(
        __x(
            "cannot chdir to source directory '{dir}': {error}",
            dir   => $config->{srcdir},
            error => $!
        )
        );
    ...
    if ( ( $modified + $deleted ) && !empty $config->{paths}->{timestamp} ) {
        my $timestamp_file = File::Spec->catfile(
            $config->{srcdir},
    ...
    $logger->info(
        __nx(
            "finished building site with one artefact",
    ...
    $logger->info(
        __nx(
            "one artefact was changed and has been re-written",
    ...
    if ($deleted) {
        $logger->info(
            __nx(
                "one stale artefact has been deleted",
...
sub dump {
    ...
    if ( 'JSON' eq uc $format ) {
        ...
        die __x(
            "Unsupported dump output format '{format}'.\n",
            format => $format
        );
    }
...
sub __startHelper {
    ...
    if ( !empty $exec ) {
        if ($>) {
            $logger->fatal(
                $log_prefix . __x(
                    "Environment variable '{variable}' ignored" . " when running as root",
...
sub _getProcessors {
    ...
    foreach my $module (@names) {
        ...
        $self->logger->fatal(
            __x(
                "invalid processor name '{processor}'",
                processor => $module
            )
...
sub scan {
    ...
    $logger->debug(
        __x(
            "scanning source directory '{srcdir}'",
    ...
    $logger->debug(
        __x(
            "scanning output directory '{outdir}'",
...
sub initAnalyzers {
    ...
    foreach my $name (@$names) {
        ...
        $self->logger->fatal(
            __x(
                "invalid analyzer name '{analyzer}'",
                analyzer => $name
            )
        ) if !perl_class $class_name;
...
sub __prune {
    ...
    my @outfiles = sort { length($b) <=> length($a) } @{ $self->{__outfiles} };
    ...
    foreach my $outfile (@outfiles) {
        if ( $directories{$outfile} || $site->getArtefact($outfile) ) {
            ...
            $logger->debug(
                __x(
                    "pruning directory '{directory}'",
                    directory => $outfile
                )
            );
            $logger->error(
                __x(
                    "cannot remove directory '{directory}': {error}",
            ...
            $logger->error(
                __x(
                    "cannot remove file '{filename}': {error}",
...
sub __filesysChangeFilter {
    ...
    if ( $config->ignorePath( $filename, 1 ) ) {
        ...
        $logger->debug(
            __x(
                "changed file '{filename}' is ignored",
                filename => $filename
            )
        );
        return;
...
sub locateAsset {
    ...
    my $render_path =
        $asset->{raw}
        ? '/' . $asset->getRelpath
        : $self->expandLink( $asset, $site, $asset->{render_path} );
    $logger->debug(
        __x(
    ...
    my ( $significant, $directory ) = fileparse $render_path;
    ...
    if ( $significant eq $asset->{index} ) {
        ...
        $asset->{'significant-path'} = $render_path;
    }
...
sub __initVersionControlled {
    ...
    foreach my $asset ( values %{ $site->{assets} } ) {
        if ( !$version_controlled->{absolute}->{ $asset->getPath } ) {
            ...
            $logger->debug(
                __x(
                    "ignoring '{relpath}': not under version control",
                    relpath => $relpath
                )
            );
...
sub __initNoSCMPatterns {
    ...
    return $config->{'no-scm'} = File::Globstar::ListMatch->new(
        $no_scm,
