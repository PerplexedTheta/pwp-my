...
sub postMeta {
    ...
    foreach my $paragraph (@paragraphs) {
        next if empty $paragraph->as_text;
        my $paragraph_words = split(/\s+/, $paragraph->as_text);
        $word_count += scalar($paragraph_words);
    }
    ...
    return excerpt => $excerpt, word_count => $word_count, read_time => $read_time, links => [keys %links];
}
