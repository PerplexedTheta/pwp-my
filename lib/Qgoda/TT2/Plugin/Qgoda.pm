...
$Qgoda::TT2::Plugin::Qgoda::VERSION = '0.9.8';
use common::sense;
...
sub __writeAsset {
    ...
    $builder->saveArtefact($asset, $site, $asset->{render_path});
    $logger->debug(__x("successfully built \"{render_path}\"",
...
sub clone {
    ...
    die __"the argument render_path is mandatory for clone()\n"
        if empty $extra->{render_path};
...
sub paginate {
    ...
    my $asset = $self->__getAsset;
    my $render_path;
    if (!empty $asset->{prev_render_path}) {
        $render_path = $asset->{prev_render_path};
    } else {
        $render_path = $asset->{render_path};
    }
    my $basename = basename $render_path;

    $basename =~ m{(.*?)(\..+)?$};
    $stem = $1 if empty $stem;
    $extender = $2 if empty $extender;

    my ($next_start, $next_render_path);
    if ($page < $total_pages) {
        $next_start = $start + $per_page;
        $next_render_path = dirname $render_path;
        $next_render_path .= "/${stem}-" . ($page + 1) . $extender;
    }
    ...
    my $retval = {
        ...
        next_render_path => $next_render_path,
    };
