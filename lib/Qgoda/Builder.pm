...
sub build {
    ...
    ASSET: foreach my $asset (sort { $b->{priority} <=> $a->{priority} }
        ...
        eval {
            ...
            if (!$options{dry_run}) {
                $self->saveArtefact($asset, $site, $asset->{render_path});
            }
...
sub processAsset {
    ...
    if (@processors) {
        ...
        $asset->{word_count} = $postmeta{word_count} if empty $asset->{word_count};
        $asset->{read_time}  = $postmeta{read_time}  if empty $asset->{read_time};
